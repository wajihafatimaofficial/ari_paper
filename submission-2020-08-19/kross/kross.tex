...
\title{\pkg{ari}: The Automated R Instructor}
...
\CRANpkg{ari}, the \textbf{A}utomated \textbf{R} \textbf{I}nstructor as
...
similar artifacts. By using \pkg{ari}, we provide a tool for users to
...
\citep{googleLanguageR, mscstts, aws.polly}. Currently in \pkg{ari},
synthesis of the the audio can be rendered using any of these services
through the \CRANpkg{text2speech} package \citep{text2speech}. The
...
implemented in the \CRANpkg{aws.polly} package \citep{aws.polly}. In
...
almost 20 years. \pkg{ari} has been built with presets for FFmpeg which
...
applied to command-line arguments FFmpeg through \pkg{ari}.
...
We will also demonstrate the \CRANpkg{ariExtra} package, which contains
functions that connect \pkg{ari} to applications outside of the R
...
installed so that \pkg{ari} can combine the audio and images, much like
packages such as \CRANpkg{animation} which have a similar requirement
\citep{aniart, anipkg}. Moreover, on
\href{https://www.shinyapps.io/}{shinyapps.io}, a dependency on the
\pkg{animation} package will trigger an installation of FFmpeg so
\pkg{ari} can be used on \href{https://www.shinyapps.io/}{shinyapps.io}.
In the example below, 2 images (packaged with \pkg{ari}) are overlaid
...
\CRANpkg{aws.signature} package \citep{aws.signature}. The
\pkg{aws.signature} documentation provides options and steps to create
...
\citep{rmarkdown, rmarkdownbook, xaringan}. In \pkg{ari}, the HTML
slides are rendered using \CRANpkg{webshot} \citep{webshot} and the
...
\pkg{rmarkdown} \citep{rmarkdown}. If the slides are already rendered,
...
\texttt{capture\_method\ =\ "vectorized"}, \pkg{webshot} is run on the
...
We created the \pkg{ariExtra}
(\url{https://github.com/muschellij2/ariExtra}) package to augment the
core functionality of \pkg{ari} by extending it to software applications
...
required for \pkg{ari}, we believed that this additional functionality
...
generation are in our package \pkg{ariExtra}. The speaker notes of
slides can be extracted using \CRANpkg{rgoogleslides}
\citep{rgoogleslides} for Google Slides via the API or using
\CRANpkg{readOffice}/\CRANpkg{officer} \citep{officer, readOffice} to
read from PowerPoint documents. Google Slides can be downloaded as a PDF
and converted to PNGs using the \CRANpkg{pdftools} package
\citep{pdftools}. The \pkg{ariExtra} package also has a
...
\CRANpkg{docxtractr} package \citep{docxtractr} which contains the
...
Alternatively, the notes can be extracted using \pkg{rgoogleslides} via
...
\pkg{av} package. Though the \pkg{av} package has powerful functionality
and is currently porting more from \texttt{libav} and therefore FFmpeg,
it currently does not have the capabilities required for \pkg{ari}.
